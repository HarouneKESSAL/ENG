...
public function index(Request $request)
{
   ...
   $today = date('Y-m-d');
    ...
    $units = Unit::with('activites', 'journals')->get();
        ...
        //             $journals = $journals->whereBetween('date', [$startDate, $endDate])->sortBy('date');  
        //         }
        //         //ZEBI KBIWWER W NIK MAMAT HMD
        //         else if ($cumule_jfm)
        //         {
        ...
        //         $journalTotals[$activite->id][$unit->id] = 
        //         [
        ...
        //             }
        //         }
        //             $journalTotals[$activite->id][$unit->id]['activitesTotals'] = $activitesTotals;
        //             $journalTotals[$activite->id][$unit->id]['journals'] = $journals; 
        ...
        foreach ($activities as $activite) {
            ...
            if ($activite->journals) {
                ...
                foreach ($activite->units as $unit) {
                    $journalTotals[$activite->id][$unit->id] =
                        [
                            'Realisation_Production' => $journals->where('unit_id', $unit->id)->sum('Realisation_Production'),
                            'Realisation_Vent' => $journals->where('unit_id', $unit->id)->sum('Realisation_Vent'),
                            'Realisation_ProductionVendue' => $journals->where('unit_id', $unit->id)->sum('Realisation_ProductionVendue'),
                            'Previsions_Production' => $journals->where('unit_id', $unit->id)->sum('Previsions_Production'),
                            'Previsions_Vent' => $journals->where('unit_id', $unit->id)->sum('Previsions_Vent'),
                            'Previsions_ProductionVendue' => $journals->where('unit_id', $unit->id)->sum('Previsions_ProductionVendue'),
                        ];
                }
    ...
    return view('admin.index', ['journalTotals' => $journalTotals, 'units' => $units, 'activities' => $activities,  'selectedDate'=>$selectedDate,'cumule'=>$cumule,'cumule_jfm'=>$cumule_jfm ]);  
...
public function show(Request $request)
{
    ...
    $activities = Activite::all();
    $activite_id = $request->query('activite_id');
...
if ($cumule) {
    ...
    return view('admin.show', [ 'journalTotals' => $journalTotals,'journals' => $journals, 'units' => $units, 'activities' => $activities,'selectedDate' => $selectedDate,'unit_id'=>$unit_id,'activite_id'=>$activite_id]);
}
