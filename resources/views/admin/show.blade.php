    ...
    <div id="app">
        ...
        <center>
            ...
            <div class="w-full p-2 overflow-hidden rounded-lg shadow-xs pt-5 pr-8">
                <div class="w-full overflow-x-auto">
                    ...
                    @if (!$hideRow)
                        ...
                        <td>
                            {{ $journal->Previsions_Production != 0 ? round(($journal->Realisation_Production / $journal->Previsions_Production) * 100, 2) : 'N/A' }}%
                        </td>

                        <td>{{ $journal->Previsions_Vent }}</td>
                        <td>{{ $journal->Realisation_Vent }}</td>
                        <td>
                            {{ $journal->Previsions_Vent != 0 ? round(($journal->Realisation_Vent / $journal->Previsions_Vent) * 100, 2) : 'N/A' }}%
                        </td>

                        <td>{{ $journal->Previsions_ProductionVendue }}</td>
                        <td>{{ $journal->Realisation_ProductionVendue }}</td>
                        <td>
                            {{ $journal->Previsions_ProductionVendue != 0 ? round(($journal->Realisation_ProductionVendue / $journal->Previsions_ProductionVendue) * 100, 2) : 'N/A' }}%
                        </td>
            ...
            @if (!empty($descriptions))
                <div class="my-4">
                    <h4 class="text-3xl font-medium pb-2">Observations:</h4>
    ...
    @if (request()->has('cumule'))
        @php
            ...
            foreach ($journalTotals as $date => $totals) {
                ...
                $vendue[] = ($totals['Realisation_ProductionVendue'] / 1000000) * 100; // calculate percentage of Realisation_ProductionVendue over 1000
            }
        @endphp
        <script src="https://cdn.jsdelivr.net/npm/chart.js"></script>
        <script>
            ...
            var myChart = new Chart(ctx, {
                ...
                data: {
                    ...
                    datasets: [{
                        label: 'Realisations',
                        data: [{!! $production[count($production) - 1] !!}, {!! $vente[count($vente) - 1] !!},
                        //  {!! $vendue[count($vendue) - 1] !!}
                        ]
                         ,
